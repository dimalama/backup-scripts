...
LOG_FILE="$LOG_DIR/obsidian_backup.log"
...
if [ ! -d "$OBSIDIAN_DIR" ]; then
    log_message "ERROR: Obsidian directory does not exist: $OBSIDIAN_DIR"
    exit 1
fi

# Navigate to the folder you want to back up
if ! cd "$OBSIDIAN_DIR"; then
    log_message "ERROR: Failed to change directory to: $OBSIDIAN_DIR"
    exit 1
fi
